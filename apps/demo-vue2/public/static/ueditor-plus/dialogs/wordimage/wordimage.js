...
function addUploadButtonListener() {
    g('saveFile').addEventListener('change', function () {
        $('.image-tip').html(lang.transferring);
...
function showLocalPath(id) {
    ...
    $('.image-tip').html('<span style="color:#ff0000;">' + lang.chooseFiles + names.join("、") + ' ' + lang.chooseFilesSuffix.replace('{#count}', images.length) + '</span>');
}
...
function createCopyButton(id, dataFrom) {
    ...
    clipboard.on('success', function (e) {
        g('copyButton').innerHTML = lang.copyDone;
    });
}
