...
(function () {
    ...
    function initVideo() {
        ...
        //编辑视频时初始化相关信息
        (function () {
            var img = domUtils.findParentByTagName(editor._shortcutmenuTarget, ["img", "video", "iframe"], true) ||
                editor.selection.getRange().getClosedNode() ||
                domUtils.findParentByTagName(editor.selection.getStart(), ["img", "video", "iframe"], true),
                url,
                className = img && img.className ? img.className : "";
            if (img) {
                var hasFakedClass = (className == "edui-faked-video"),
                    hasUploadClass = className.indexOf("edui-upload-video") != -1,
                    hasIframeClass = className.indexOf("edui-video-iframe") != -1 || img.tagName === "IFRAME",
                    hasVideoClass = className.indexOf("edui-video-video") != -1 || img.tagName === "VIDEO";
                if (hasFakedClass || hasUploadClass) {
                    $G("videoUrl").value = url = img.getAttribute("_url") || img.getAttribute("src");
                ...
                if (hasUploadClass || hasIframeClass || hasVideoClass) {
    ...
    function getFileVideoSize(file) {
        return new Promise(function (resolve) {
            if (!file) {
                resolve({width: 0, height: 0});
                return;
            }
            var source = file.source && file.source.source ? file.source.source : file;
            var objectUrl = '';
            try {
    ...
    function insertUpload() {
        ...
        for (var key in uploadVideoList) {
            ...
            videoObjs.push({
                ...
                width: width || file.width || 420,
                height: height || file.height || 280,
                align: align
            });
        }
        ...
        if (count) {
            $('.info', '#queueList').html('<span style="color:red;">' + lang.queueError.replace(/_/, count) + '</span>');
            return false;
        } else {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        getFileVideoSize(file).then(function (size) {
